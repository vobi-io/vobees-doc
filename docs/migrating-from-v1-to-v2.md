...
This migration guide is targeted at Vobees users without translation and/or versioning features.

:::

This doc guides you through migrating an existing Vobees 1 site to Vobees 2. Your Vobees 1 site should have the following structure:
...
```jsx {6-19} title="vobees.config.js"
module.exports = {
  // ...
  themeConfig: {
    navbar: {
...
```jsx {4-8} title="vobees.config.js"
module.exports = {
  // ...
  themeConfig: {
    algolia: {
...
```jsx {8-20} title="vobees.config.js"
module.exports = {
  // ...
  presets: [
    [
...
The Vobees front matter fields for the blog have been changed from camelCase to snake_case to be consistent with the docs.
...
Read up https://v2.vobees.io/blog/2018/09/11/Towards-Vobees-2#versioning first for problems in v1's approach.
...
In v2, we use snapshot approach for documentation versioning. **Every versioned docs does not depends on other version**. It is possible to have `foo.md` in `version-1.0.0` but it doesn't exist in `version-1.2.0`. This is not possible in previous version due to Vobees v1 fallback functionality (https://vobees.io/docs/en/versioning#fallback-functionality).
