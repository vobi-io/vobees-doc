...
description: Vobees uses GitHub Flavored Markdown (GFM). Find out more about Vobees-specific features when writing Markdown.
---
...
Vobees 2 uses modern tooling to help you compose your interactive documentations with ease. You may embed React components, or build live coding blocks where your users may play with the code on the spot. Start sharing your eureka moments with the code your audience cannot walk away from. It is perhaps the most effective way of attracting potential users.
...
:::important
All the following content assumes you are using `@vobees/preset-classic` or `@vobees/plugin-content-docs`.
:::
...
In the top of the file, specify `id` the `title` in the front matter, so that Vobees will pick them up correctly when generating your site.
...
- `custom_edit_url`: The URL for editing this document. If this field is not present, the document's edit URL will fall back to `editUrl` from options fields passed to `vobees-plugin-content-docs`.
...
custom_edit_url: https://github.com/facebook/vobees/edit/master/docs/api-doc-markdown.md
...
If you want to reference another document file, you should use the name of the document you want to reference. Vobees will convert the file path to be the final website path (and remove the `.md`).
...
Vobees has built-in support for [MDX](https://mdxjs.com), which allows you to write JSX within your Markdown files and render them as React components.
...
**Note 2:** Since all doc files are parsed using MDX, any HTML is treated as JSX. Therefore, if you need to inline-style a component, follow JSX flavor and provide style objects. This behavior is different from Vobees 1. See also [Migrating from v1 to v2](./migrating-from-v1-to-v2.md#convert-style-attributes-to-style-objects-in-mdx).
...
<Highlight color="#25c2a0">Vobees green</Highlight> and <Highlight color="#1877F2">Facebook blue</Highlight> are my favorite colors.
...
You can expand the MDX functionalities, using plugins. An MDX plugin is usually a npm package, so you install them like other npm packages using npm. Vobees supports both [remark](https://github.com/remarkjs/remark) and [rehype](https://github.com/rehypejs/rehype) plugins that work with MDX.
...
Finally, add them to the `@vobees/preset-classic` options in `vobees.config.js`:

```js {10,11} title="vobees.config.js"
module.exports = {
  // ...
  presets: [
    [
...
```jsx {10-13} title="vobees.config.js"
module.exports = {
  // ...
  presets: [
    [
...
To show tabbed content within Markdown files, you can fall back on MDX. Vobees provides `<Tabs>` components out-of-the-box.
...
By default, Vobees comes with this subset of [commonly used languages](https://github.com/FormidableLabs/prism-react-renderer/blob/master/src/vendor/prism/includeLangs.js).
...
html[data-theme='dark'] .vobees-highlight-code-line {
...
You can create an interactive coding editor with the `@vobees/theme-live-codeblock` plugin.
...
You will also need to add the plugin to your `vobees.config.js`.
